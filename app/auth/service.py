...
self_space_id = 'appnamehere'
domain="user"

def do_jwttest(tenant):
    tenant=get_collection('appnamehere', 'tenant').find_one({'name': tenant})
...
def do_signin_via_jwt(tenant, data):
    tenantData=get_collection('appnamehere', 'tenant').find_one({'name': tenant})
